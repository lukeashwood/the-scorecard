...
| `controversies.html` | Documented ministerial controversies since May 2022 from hand-verified `data/controversies.json`: what happened, the minister's response, the outcome (finding, apology, repayment, resignation, referral or none), category, at least two reputable sources per entry. Filter by category, minister and outcome. Pipeline validates, link-checks and publishes `data/controversies.js`. |
| `laws.html` | Federal legislation under the current government in five statuses (scheduled, debated, passed, failed, repealed), newest first, from hand-verified `data/laws.json`: official aph.gov.au link, plain summary, and where applicable an attributed controversy note (category, who objected, what they argued, the government's reply, sources) and, where it exists, a "What the public said" note with measurable public-opinion evidence (polls, petition counts, protest turnouts, inquiry submission numbers, referendum results), plus a "Have your say" button that pre-fills the suggestion form. The pipeline validates the file, link-checks every URL, flags overdue re-checks and publishes `data/laws.js`. |
